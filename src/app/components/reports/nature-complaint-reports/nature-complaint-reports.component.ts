...
@Component({
  selector: 'app-nature-complaint-reports',
  templateUrl: './nature-complaint-reports.component.html',
  styleUrls: ['./nature-complaint-reports.component.scss'],
  providers: [MasterService, NgxSpinnerService, CommonHelper, DatePipe]

})
export class NatureComplaintReportsComponent implements OnInit {
  reportsNatureComplainData: any;
  ...
  ngOnInit() {
    ...
    this.reportsNatureComplainData = [];
  ...
  getReports(param: string) {
    ...
    this.apiCallForPostData("complaints/nature/reports", data, param);
  }
  ...
  apiCallForPostData(url: string, data: any, param: string) {
    ...
    this.masterService.postData(url, data).subscribe((response: any) => {
      if (response.status) {
        if (url === 'complaints/nature/reports') {
...
postDataBinding(response: any, param: string) {
  ...
  if (response.length > 0) {
    ...
    if (param === 'isExcel' || param === 'isPdf') {
      ...
      this.reportsNatureComplainData = this.vals;
    }
  } else {
    this.reportsNatureComplainData = [];
    this.spinner.hide();
...
exportToExcel(data: any, options: any) {
  new Angular5Csv(data, CommonConstants.natureComplaintWiseReport, options);
}
...
exportToPdf(data: any) {
  ...
  var header = function (data) {
    ...
    doc.text("Grievance Nature Report", 220, 20);
  };
  ...
  doc.save(CommonConstants.natureComplaintWiseReport + '.pdf');
}
