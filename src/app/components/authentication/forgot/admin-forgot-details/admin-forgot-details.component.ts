...
@Component({
  selector: 'app-admin-forgot-details',
  templateUrl: './admin-forgot-details.component.html',
  styleUrls: ['./admin-forgot-details.component.scss'],
  styles: [`
  #captcha{
...
export class AdminForgotDetailsComponent implements OnInit {
    ...
    try {
      ...
      if (this.router.url === '/forgot-username') {
        ...
        data = {
          ...
          EmailOrMobile: apiData.mobileNumber,
      ...
      this.authenticationService.postData('user/' + url, data).subscribe((response: any) => {
        if (response.status) {
          localStorage.setItem("mobileNo", apiData.mobileNumber);
          this.spinner.hide();
          this.commonHelper.toasterMessage(response.message);
          if (this.router.url === '/admin-forgot-password') {
  ...
  passwordConfirmation() {
    try {
      ...
      let apiData = this.confirmForgotPasswordForm.value,
        data = {
          EmailOrMobile: localStorage.getItem("mobileNo"),
          password: apiData.newPassword,
          confirmPassword: apiData.confirmPassword,
          otp: apiData.forgotPasswordOTP
        }
      this.spinner.show();
      this.authenticationService.postData('user/forgot_password', data).subscribe((response: any) => {
        if (response.status) {
          this.spinner.hide();
          this.commonHelper.toasterMessage(response.message);
          this.router.navigateByUrl('/login');
        ...
        if (error.status === 422) {
          ...
          if (error.error.errors.otp) {
            this.commonHelper.toasterMessageError(error.error.errors.otp[0]);
          }
        }
      });
    } catch (exc) {
