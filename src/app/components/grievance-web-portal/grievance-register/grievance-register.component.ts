...
@Component({
  selector: 'app-grievance-register',
  templateUrl: './grievance-register.component.html',
  styleUrls: ['./grievance-register.component.scss'],
  providers: [MasterService, DatePipe, CommonHelper]
})
export class GrievanceRegisterComponent implements OnInit {
  placeData: any;
  severityData: any;
  feedBackData: any;
  statesData: any;
  ...
  ngOnInit() {
    ...
    this.masterService.getLang.subscribe(resp => {
  ...
  */
  ...
  onChangeOfCategory(event) {
    this.registerGrevianceForm.controls['filterGrievanceNature'].reset();
    this.getGrievanceNature(event.grievanceTypeId);
  }

  onNatureChange(event) {
    this.getGrievanceCategory(event.id)
  }

  // function for getting grievanceNature
  getGrievanceCategory(id) {
    //this.apiCalling('active_grievanceNature', id)
    try {
      this.spinner.show();
      this.masterService.getData('active_grievanceCategory/', id).subscribe((response: any) => {
        if (response.status) {
          let newData = [];
          this.spinner.hide();
          response.data.filter(newResponse => {
            newData.push({ id: newResponse.grievanceCategoryId, name: newResponse.grievanceCategoryName });
          });
          if (newData.length > 0) {
            this.spinner.hide();
            this.gCategory = newData;
          } else {
            this.gCategory = [];
          }
          this.spinner.hide();
  ...
  onCategoryChange(value) {
    this.selCategory = value.id
    this.getGrievanceSubcategory(this.selCategory)
  }

  // function for getting grievanceNature
  getGrievanceSubcategory(id) {
    try {
      this.spinner.show();
      this.masterService.getData('active_grievancesubCategory/', id).subscribe((response: any) => {
        if (response.status) {
          let newData = [];
          this.spinner.hide();
          response.data.filter(newResponse => {
            newData.push({ id: newResponse.grievanceSubCategoryId, name: newResponse.grievanceSubCategoryName });
          });
          if (newData.length > 0) {
            this.spinner.hide();
            this.sCategory = newData;
          } else {
            this.sCategory = [];
          }
          this.spinner.hide();
 ...
 */
  ...
  */
  emptyByDefault() {
    ...
    this.registerGrevianceForm.get('addGrievanceCat').setValue([]);
    this.registerGrevianceForm.get('addGrievanceSubCat').setValue([]);
  }
  ...
  grievanceRegisterValidations(mobileNo: any, name: string) {
    ...
    if (this.addGrievance == true) {
      this.registerGrevianceForm = this.fb.group({
        ...
        filterGrievanceNature: this.categoriesNature.length > 0 ? [[]] : [''],
        addGrievanceCat: [],
        addGrievanceSubCat: [],
        ...
        filterVillageName: [[], Validators.required],
        filterPlaceName: [[], Validators.required],
      ...
      this.registerGrevianceForm = this.fb.group({
        ...
        filterGrievanceNature: [this.editGrievanceData.grievanceNatures],
        addGrievanceCat: [this.editGrievanceData.grievanceNatures],
        addGrievanceSubCat: [this.editGrievanceData.grievanceNatures],
    ...
    try {
      ...
      if (this.addGrievance == true) {
        data = {
          ...
          grievanceNatureId: apiData.filterGrievanceNature == null ? "" : apiData.filterGrievanceNature.id,
          grievanceCategoryId: apiData.addGrievanceCat == null ? "" : apiData.addGrievanceCat.id,
          grievanceSubCategoryId: apiData.addGrievanceSubCat == null ? "" : apiData.addGrievanceSubCat.id,
        ...
        this.apiCallingForPostData('beneficiary/register_grievance', data)
  ...
  apiCallingForPostData(url: string, data: any) {
    try {
      ...
      this.masterService.postData(url, data).subscribe((response: any) => {
        ...
        if (response.status) {
          this.spinner.hide();
          if (url === 'beneficiary/register_grievance') {
            this.commonHelper.toasterMessage(response.message);
            this.openViewModal()
          } else if (url === 'helpdesk/register_grievance') {
            this.commonHelper.toasterMessage(response.message);
            this.openViewModal()
          }
