...
export class HelpdeskDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    let apiUrl = "helpdesk/grievance_list_all";
    this.getAllGrievanceCount('helpdesk/total/count');
    this.getData(apiUrl)
  }
  ...
  */
  getAllGrievanceCount(apiRoute: string) {
    try {
      // this.spinner.show();
      this.masterService.getHelpDeskCount(apiRoute).subscribe(
        (response: any) => {
  ...
  getData(apiRoute: string) {
    ...
    try {
      this.spinner.show();
      this.masterService.getHelpDesk(apiRoute).subscribe(
        (response: any) => {
          if (response.status) {
            if (response.data.length > 0) {
              this.spinner.hide();
              ...
              this.dataTable = table.DataTable({ order: [] }).draw();
            } else {
              this.spinner.hide();
